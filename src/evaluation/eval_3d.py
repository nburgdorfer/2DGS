...
def dtu_point_eval(cfg, scan, method):
    ...
    mask_th = cfg["point_cloud"]["mask_th"]
    max_dist = cfg["point_cloud"]["max_dist"]
    min_dist = cfg["point_cloud"]["min_dist"]
    min_point_dist = cfg["point_cloud"]["min_point_dist"]
    resolution = cfg["point_cloud"]["resolution"]
    scan_num = int(scan[-3:])
    ...
    print(f"Completeness: {comp:0.4f}")
    print(f"F-Score: {fscore:0.4f}")
    print(f"Elapsed time: {dur:0.3f} s")
    ...
    with open(stats_file, 'w') as f:
        ...
        f.write(f"Completness: {comp:0.3f}mm\n")
        f.write(f"Overall: {(acc+comp)/2.0:0.4f}\n")
        f.write(f"Precision: {prec:0.3f}\n")
        f.write(f"Recall: {rec:0.3f}\n")
        f.write(f"F-Score: {fscore:0.3f}\n")

    return acc, comp, prec, rec, fscore
