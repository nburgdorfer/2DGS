...
class GaussianExtractor(object):
    ...
    def export_image(self, path):
        rendered_image_path = os.path.join(path, "rendered_image")
        ...
        for idx, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="export images"):
            ...
            write_pfm(os.path.join(depth_path, f"{idx:08d}.pfm"), depth)
            cv2.imwrite(os.path.join(depth_path, f"{idx:08d}.png"), 255*(depth-depth.min()) / (depth.max()-depth.min()+1e-10))

            write_pfm(os.path.join(normal_path, f"{idx:08d}.pfm"), normal)
            cv2.imwrite(os.path.join(normal_path, f"{idx:08d}.png"), 255*(normal-normal.min()) / (normal.max()-normal.min()+1e-10))
